...
class ClassificationGrader:
    ...
    def grade_step(
        ...
        if action_type == "request_info":
            ...
            return round(max(0.0, reward), 3), info

        return 0.0, {"note": "no reward for this action in classification task"}
...
class ResolutionGrader:
    ...
    def grade_step(
        ...
        if action_type == "request_info":
            questions = payload.get("questions", [])
            reward = 0.05 if self.required_info and questions else 0.0
            return reward, info
